...
use crate::{analyzer_service::{
    ...
    InitResponse, 
    Location, 
    NotifyFileChangesRequest, 
    NotifyFileChangesResponse, 
    Position, 
...
impl ProviderService for CSharpProvider {
    async fn capabilities(        
        ...
        return Ok(Response::new(CapabilitiesResponse{capabilities: vec![Capability{ name: "referenced".to_string(), template_context: None }]})); 
    }
    ...
    async fn init(
        ...
        let config = r.get_ref();
        println!("got config: {:?}", config);
        if self.config.lock().await.is_some() {
            return Err(Status::already_exists("already initialized"))
        }
        let _ = self.config.lock().await.insert(config.clone());   




        println!("db_path {:?}", self.db_path);
        let path = PathBuf::from(config.location.as_str());
        let stats = load_database(path, self.db_path.to_path_buf());
        println!("loaded files: {:?}", stats);


        return Ok(Response::new(InitResponse { error: "".to_string(), successful: true, id: 4, builtin_config: None }));
    }
    ...
    async fn evaluate(
        ...
        let condition: CSharpCondition = match serde_yml::from_str(evaluate_request.condition_info.as_str()) {
        ...
        let search = FindNode{ node_type: condition.referenced.location, regex: condition.referenced.pattern };
        let results = match search.run(&self.db_path) {
            Ok(res) => {
                // TODO convert Vec<Result> to ProviderEvaluateResponse
                if res.is_empty() {
                    EvaluateResponse{ error: "".to_string(), successful: true, response: Some(ProviderEvaluateResponse{
                        matched: false,
                    ...
                    EvaluateResponse{error: "".to_string(), successful: true, response: Some(ProviderEvaluateResponse{
                         matched: true, 
                         incident_contexts: res.iter().map(|r| {
                            IncidentContext{ 
                                ...
                                variables: Some(Struct{ fields: r.variables.clone()}),
                                links: vec![], 
            ...
            Err(err) => {
                ...
                EvaluateResponse{ error: err.to_string(), successful: false, response: None }
            },
        };
    ...
    async fn get_dependencies(
        ...
        return Ok(Response::new(DependencyResponse { successful: true, error: "".to_string(), file_dep: vec![] }));
    }
    ...
    async fn get_dependencies_dag(
        ...
        return Ok(Response::new(DependencyDagResponse { successful: true, error: "".to_string(), file_dag_dep: vec![] }));
    }
    ...
    async fn notify_file_changes(
        ...
        return Ok(Response::new(NotifyFileChangesResponse{ error: "".to_string()}));
    }
}